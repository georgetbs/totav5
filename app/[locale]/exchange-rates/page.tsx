            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{translate('exchange.date', locale)}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {rates.map((rate) => (
                  <TableRow key={rate.code}>
                    ...
                    <TableCell className={rate.diff > 0 ? 'text-green-600' : rate.diff < 0 ? 'text-red-600' : ''}>
                      {rate.diff !== 0 ? (
                        <>
                          {rate.diff > 0 ? <ArrowUp className="inline h-4 w-4 mr-1" /> : <ArrowDown className="inline h-4 w-4 mr-1" />}
                          {rate.diffFormated}
                        </>
                      ) : '-'}
                    </TableCell>
                    <TableCell>{formatDate(new Date(rate.date), locale)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
