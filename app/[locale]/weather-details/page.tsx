  ...
  if (
    ...
    return (
      <div className="relative w-6 h-6">
        ...
        <Sun className="absolute w-4 h-4 text-yellow-500 z-0 top-0 right-0 opacity-100" />
      </div>
    )
  }
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        const [forecastData, currentData] = await Promise.all([forecastResponse.json(), currentResponse.json()])
          ...
          <div className="space-y-8">
            ...
            <div className="bg-muted/50 rounded-lg p-6">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-6">
                  <div className="text-6xl font-bold">{formatTemperature(currentWeather.main.temp)}°C</div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {translate("weather.feelsLike", locale)}: {formatTemperature(currentWeather.main.feels_like)}°C
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Droplets className="w-5 h-5 text-muted-foreground" />
                    <span>
                      {translate("weather.humidity", locale)}: {currentWeather.main.humidity}%
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Wind className="w-5 h-5 text-muted-foreground" />
                    <span>
                      {translate("weather.windSpeed", locale)}: {currentWeather.wind.speed}{" "}
                      {translate("weather.metersPerSecond", locale)}
                    </span>
                  </div>
                </div>
              </div>
                ...
                {hourlyForecast.map((hour, index) => (
                  <Card key={index} className="text-center">
                    <CardContent className="p-4 flex flex-col items-center">
                      ...
                      <p className="text-xl font-bold mb-2">{formatTemperature(hour.main.temp)}°C</p>
                  ...
                  <Card key={index}>
                    <CardContent className="p-4">
                      <div className="grid grid-cols-2 sm:grid-cols-5 items-center gap-2">
                        <p className="font-medium">{translate(`weather.days.${day.date.getDay()}`, locale)}</p>
                        <div className="flex justify-center">{getWeatherIcon(day.weather.description)}</div>
                        <p className="text-center">
                          <span className="font-bold">
                            {day.dayTemp !== null ? formatTemperature(day.dayTemp) : "—"}°C
                          </span>
                          <br />
                          <span className="text-sm text-muted-foreground">{translate("weather.day", locale)}</span>
                        </p>
                        <p className="text-center">
                          <span className="font-bold">
                          ...
                          <br />
                          <span className="text-sm text-muted-foreground">{translate("weather.night", locale)}</span>
                        </p>
                        <p className="text-muted-foreground text-center sm:text-right col-span-2 sm:col-span-1 mt-2 sm:mt-0">
            ...
            {lastUpdated && (
              <div className="text-sm text-muted-foreground text-center">
                <p>
                  {translate("weather.lastUpdated", locale)}: {lastUpdated.toLocaleString(locale)}
                </p>
              </div>
            )}
          </div>
